...
const [friends, setFriends] = useState([]);
  ...
  //FetchFriends for  the split bill feature
  useEffect(() => {
  const fetchFriends = async () => {
    const querySnapshot = await getDocs(collection(db, 'users'));
    const usersList = querySnapshot.docs.map(doc => doc.data().name); // or doc.data().email
...
useEffect(() => {
  ...
  bills.forEach((bill) => {
    const billDate = bill.createdAt?.toDate?.(); // Firestore Timestamp
      ...
      <div className="flex flex-col w-full p-4 pb-20 md:pb-4">
        ...
        <div className="flex flex-col gap-6 lg:flex-row">
          ...
          <div className="flex flex-col flex-1 gap-4">
            {/* Average Spend Card */}
            <div className="bg-gradient-to-br from-[#544ECD] to-[#2A2767] text-white rounded-2xl shadow-xl p-4 md:p-6 min-h-[180px]">
              <div className="flex flex-col items-center gap-3 md:flex-row md:items-start">
                ...
                <div className="bg-[#eeeeffd4] rounded-lg p-3 shadow-lg text-[#3C3E87] ml-6">
                  <h3 className="mb-4 font-normal -2 text-md">My Summary (This Month)</h3>
            ...
            <div className="flex flex-col gap-4 md:flex-row">
              <div className="flex flex-col justify-between bg-[#EEEEFF] p-4 md:p-6 rounded-2xl shadow-lg flex-1">
                <div>
              ...
              <div className="flex flex-col justify-between bg-[#EEEEFF] flex-1 p-4 md:p-6 shadow-lg rounded-2xl">
                <div>
                ...
                <button 
                  className="px-4 py-2 mt-4 text-sm text-white bg-gradient-to-br from-[#991919] to-[#FF2929] rounded-xl hover:brightness-90 w-fit"
          ...
          {/* Transactions Panel */}
          <div className="flex-1 p-4 rounded-2xl md:p-6 bg-[#EEEEFF]">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left min-w-[400px]">
                ...
                <tbody className="text-[#3C3E87]">
                  {bills.map((bill) => (
                    <>
                      <tr key={bill.id} className="h-12">
                        <td className="flex gap-1">
                          {bill.splitTo && bill.splitTo.length > 0 ? (
                            bill.splitTo.map((name, idx) => (
                              <span
                                key={idx}
                                className="bg-[#DADAFE] text-[#3C3E87] px-2 py-1 rounded-full text-xs font-semibold"
                              >
                                {name.split(' ').map(w => w[0]).join('').toUpperCase()}
                              </span>
                            ))
                          ) : (
                            <span className="bg-[#DADAFE] text-[#3C3E87] px-2 py-1 rounded-full text-xs font-semibold">
                        ...
                        <td>
                          <button
                            ...
                            className={`px-3 py-1 text-sm text-white rounded-full ${bill.status === 'Paid' ? 'bg-green-500' : 'bg-red-500'}`}
                          >
                            {bill.status || 'Pending'}
                          </button>
                        </td>
                      ...
                      {expandedBillId === bill.id && (
                        <tr>
                          <td colSpan="5" className="px-4 py-2 bg-white/50">
                            <div className="flex flex-col gap-2">
                              <h5 className="font-medium">Payment Details:</h5>
                              <ul className="flex flex-wrap gap-2">
                                {bill.splitTo?.map((person, idx) => (
                                  <li key={idx} className="flex items-center gap-2 px-3 py-1 bg-[#F5F5FF] rounded-full">
                                    <span>{person}</span>
                                    <button
                                      onClick={() => togglePaymentStatus(bill.id, person, bill.paidBy || [])}
                                      className={`px-2 py-0.5 text-xs rounded-full ${(bill.paidBy || []).includes(person) ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}
                                    >
  ...
  friends={friends} // Pass friends to modal
  onSubmit={async (billData) => {
    try {
      const docRef = await addDoc(collection(db, 'bills'), {
        ...
        paidBy: [],
        createdAt: new Date(),
        createdBy: currentUser // Add who created the bill
      });
